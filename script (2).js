...
const renderFacultyList = () => {
    ...
    currentFaculty.forEach(f => {
        list.innerHTML += `<li>
            <span><i class="fa-solid fa-user text-secondary"></i> <strong>${f.name}</strong> (${f.department})</span>
            <button class="btn delete-btn" onclick="deleteFaculty('${f.id}')" title="Delete Faculty">
...
window.deleteFaculty = async (id) => {
    if(confirm("Are you sure you want to delete this faculty? This action cannot be undone.")) {
        await fetchAPI(`/faculty/${id}`, { method: 'DELETE' });
